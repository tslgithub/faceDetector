...
def inference(net,img,im_height, im_width,cfg):
    loc, conf, landms = net(img)  # forward pass

    priorbox = PriorBox(cfg, image_size=(im_height, im_width))
    priors = priorbox.forward()
    return conf,landms,priors,loc

def postProcess(args,scale,conf,landms,priors,cfg,img,device,loc):
    resize = 1
...
def fixResult(args,dets,img_raw,file,im_height, im_width):
    num = 0
    if args.save_image:
        for b in dets:
            ...
            text = "{:.4f}".format(b[4])
            b = list(map(int, b))
            cv2.rectangle(img_raw, (b[0], b[1]), (b[2], b[3]), (0, 0, 255), 3)
            cx = b[0]
            cy = b[1] + 12
            cv2.putText(img_raw, text, (cx, cy),cv2.FONT_HERSHEY_DUPLEX, 0.7, (255, 255, 255))
            # print("{} @ ({} {} {} {})".format(text,b[0], b[1],b[2], b[3]) )

            # drawLandMark(img_raw,b)
        print("{}\t{}".format(file,num))
        # save image
        cv2.putText(img_raw, "total: "+str(num), (10,50),cv2.FONT_HERSHEY_DUPLEX, 1*(im_height* im_width)/(640*480)*0.5, (255, 0, 0),2)
